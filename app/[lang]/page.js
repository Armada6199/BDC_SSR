  ...
  async function handleLogin() {
    try {
      ...
      if (loginResponse.error) {
        ...
        const mockData = session.userData.employeeData;
        const init=0;
        const maxAmountAfterDeduction=mockData.activeLoans.reduce(
          (accumulator, currentValue) => accumulator + currentValue.activeDeductedAmount,
          init,
        );
        setCurrentLoan((prev) => ({ ...prev, ...mockData, isStaff: true,maxAmountAfterDeduction}));
        setIsLogingin(false);
        enqueueSnackbar(`Welcome Back ${mockData.employeeName}`, {
          variant: "success",
